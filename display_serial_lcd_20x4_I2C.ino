//***********************************************************************************************
//Original code by Daniel Barker for 16x2 LCD display available at 
//https://github.com/djbarker/cpu-status-display
//
//Code modified by Don B. for 20x4 I2C LCD display using 
//NewLiquidCrystal available at https://bitbucket.org/fmalpartida/new-liquidcrystal/wiki/Home
//by F. Malpartida.
//************************************************************************************************
...
/*
   Parameters
*/
#define MAX_LEN 275 // maximum line length
...
/*
   Variables
*/
#define I2C_ADDR      0x3f // define your I2C address of PCF8574A
...
void loop()
{
  ...
  if ((millis() - last_refresh) >= refresh && had_data)
  {
    ...
    if (len1 <= 20) memcpy(tmp, line1, 20);
    else         memcpyp(tmp, line1, 20, pos1, len1, ' ', 4);

    lcd.setCursor(0, 0);
    lcd.print(tmp);

    if (len2 <= 20) memcpy(tmp, line2, 20);
    else         memcpyp(tmp, line2, 20, pos2, len2, ' ', 4);
  ...
  {
    if (len1 > 20) pos1 = (pos1 + 1) % (len1 + 4);
    if (len2 > 20) pos2 = (pos2 + 1) % (len2 + 4);
